...
public final class Features {
    private static final Map<Class<?>, Object> FEATURES = new HashMap<Class<?>, Object>(4);
    static {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            FEATURES.put(StrictModeFeature.class, new HoneycombStrictModeFeature());
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
            FEATURES.put(StrictModeFeature.class, new GingerbreadStrictModeFeature());
        } else {
    ...
    public static <T> T getFeature(Class<T> featureClass) {
        ...
        if (feature == null) {
            throw new IllegalArgumentException("Unsupported feature: " + featureClass.getName());
        }
        return feature;
    }
}
